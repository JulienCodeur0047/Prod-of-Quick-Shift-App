...
const ShiftRow: React.FC<{ shift: Shift }> = ({ shift }) => {
    ...
    return (
        <div className="bg-blue-100 dark:bg-blue-900/50 p-3 rounded-lg">
            ...
            {shift.locationId && <p className="text-xs text-slate-600 dark:text-slate-400">Loc: {shift.locationId}</p>}
        </div>
    );
};

const DayCard: React.FC<{ date: Date, shifts: Shift[], t: (key: string) => string }> = ({ date, shifts, t }) => {
        ...
        <div className="bg-white dark:bg-blue-night-900 p-3 rounded-lg shadow-sm">
            ...
            {shiftsForDay.length > 0 ? (
                <div className="space-y-2">
                    {shiftsForDay.map(shift => <ShiftRow key={shift.id} shift={shift} />)}
                </div>
            ) : (
...
const ScheduleScreen: React.FC = () => {
    ...
    return (
        <div className="p-4 md:p-6">
            <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-6">{t('mobile.upcomingShifts')}</h1>
            
            <div className="space-y-6">
                <div>
                    <h2 className="font-bold text-lg text-slate-700 dark:text-slate-200 mb-3">{t('mobile.currentWeek')}</h2>
                    <div className="space-y-3">
                        {currentWeekDates.map(date => <DayCard key={date.toISOString()} date={date} shifts={shifts} t={t} />)}
                    </div>
                </div>
                <div>
                    <h2 className="font-bold text-lg text-slate-700 dark:text-slate-200 mb-3">{t('mobile.nextWeek')}</h2>
                    <div className="space-y-3">
                        {nextWeekDates.map(date => <DayCard key={date.toISOString()} date={date} shifts={shifts} t={t} />)}
                    </div>
                </div>
            </div>
        </div>
